...
def get_url(url):
    ...
    try:
        world.last_response = urllib.request.urlopen(url)
...
@step('my settings.py has "(.*)" set to "(.*)"')
...
@step('When all the responses have status code 200')
...
@step('Then all the responses have mime type "(.*)"')
...
@step('try to resolve the assets provided by given HTML')
...
@step('all the responses have are between one of those mime types:')
...
def should_be_an_image(step, expected):
    ...
    assert expected in got, \
        'fetching the url %s, the content-type header should contain "%s", ' \
